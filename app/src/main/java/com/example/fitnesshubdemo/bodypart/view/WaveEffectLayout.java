package com.example.fitnesshubdemo.bodypart.view;
...
import com.example.fitnesshubdemo.bodypart.region.Region;
import com.example.fitnesshubdemo.bodypart.region.RegionParam;
import com.example.fitnesshubdemo.bodypart.region.RegionPathView;
...
public class WaveEffectLayout extends FrameLayout implements Runnable {
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        int regionW = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 50, getResources().getDisplayMetrics());
        ;
...
//        canvas.clipRect(left, top, right, bottom);
    ...
    private void initParametersForRegionLocation() {
        int middleAlignmentX = this.getWidth() / 2 - this.getPaddingLeft();
    ...
    private int touchPointInRegion(int x, int y) {
        ...
        if (HumanBodyWidget.mShowingBack) {
            return RegionParam.REGION_BACK;
        } else {
            return RegionParam.REGION_FRONT;
        }
